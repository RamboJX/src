...

{
  ...
  if (options.length() > 0) {
		...
		for( i = 0; i < length; ++i ){
            ...
            if( theOption[0] == MString("groups") &&theOption.length() > 1 ) {
            ...
            if( theOption[0] == MString("materials") && theOption.length() > 1 ) {
                if( theOption[1].asInt() > 0 ){
                    materials = true;
                }else{
                    materials = false;
            ...
            if( theOption[0] == MString("ptgroups") && theOption.length() > 1 ) {
                if( theOption[1].asInt() > 0 ){
                    ptgroups = true;
                }else{
                    ptgroups = false;
            ...
            if( theOption[0] == MString("normals") && theOption.length() > 1 ) {
                if( theOption[1].asInt() > 0 ){
                    normals = true;
                }else{
                    normals = false;
            ...
            if( theOption[0] == MString("smoothing") && theOption.length() > 1 ) {
                if( theOption[1].asInt() > 0 ){
                    smoothing = true;
                }else{
                    smoothing = false;
...
)
{
	...
	//write out the mesh object node name
	fprintf(fp,"o %s\n",mdagPathNodeName.asChar(),objectIdx);
...
//
{
	...
	for ( i=0; i<length; i++ )
	{
		...
		MFnSet fnSet( mset, &stat );			//建立set的函数集，判断set是什么类型的
		if ( stat ) {
			if ( MFnSet::kRenderableOnly == fnSet.restriction(&stat) ) {
				if ( materials ) {				//如果set是material
					sets->append( mset );
				}
			} 
			else {
				if ( groups ) {					//如果set是其他的group
					sets->append( mset );
				}
			}
		}	
